...
/**
 * Created by luozheng on 2016/10/24.
 */
...
final public class SharedPrefsUtils {
    ...
    public static boolean setObjectPreference(Context context, String key, Object obj) {
        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(context);
        if (preferences != null && !TextUtils.isEmpty(key)) {
            SharedPreferences.Editor editor = preferences.edit();
            editor.putString(key, GsonUtils.getInstance().get().toJson(obj));
            return editor.commit();
        }
        return false;
    }

    public static Object getObjectPreference(Context context, String key,Class clazz) {
        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(context);
        if (preferences != null) {
            Object value = GsonUtils.getInstance().get().fromJson(preferences.getString(key,""),clazz);
            return value;
        }
        return null;
    }
